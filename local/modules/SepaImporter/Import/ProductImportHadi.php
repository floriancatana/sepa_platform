 ...
 */
class ProductImportHadi extends AbstractImport {
    /* @var Tlog $log */
    ...
    private function getCategoryIds($id) {
        $url = "http://www.eclasscontent.com/index.php?id=" . $id . "&4action=&language=de&searchtxt=&options=&version=10.0.1";
        $ch = curl_init();
        ...
        $str = htmlentities($response);
        curl_close($ch);

        preg_match_all('/[$].*node_[0-9]*/', $str, $matches);
        $matches = array_unique($matches[0]);
        foreach ($matches as $key => $val) {
            $matches[$key] = str_replace('$(&quot;#node_', "", $val);
        }

        return $matches;
    }

    private function getSurnameCode($id) {
        $url = "http://www.eclasscontent.com/index.php?action=search&language=de&version=10.0.1&options=class&searchtxt=" . $id;
        ...
        preg_match_all('/(.+)td(.+)[a-zA-z]*td(.+)/', $str, $matches);

        if (isset($matches[0][1])) {
            $matches = trim($matches[0][1]);
            $matches = str_replace("&lt;td&gt;", "", $matches);
            $matches = str_replace("&lt;/td&gt;", "", $matches);
        } else {
            return NULL;
        }
    ...
    protected function saveIsNotInCategory($name, $kategorie_id) {
        $category = CategoryI18nQuery::create()
                ->findOneByTitle($name);
        ...
        if (!$category) {
            ...
            $newCategoryInDe->setTitle($name);

            $newCategory = new Category();
            $newCategory->setParent($this->parentId);
            $newCategory->setVisible(1);
            $newCategory->addCategoryI18n($newCategoryInEn);
            $newCategory->addCategoryI18n($newCategoryInDe);
            $newCategory->save();
            $this->parentId = $newCategory->getId();
        } else {
            $this->parentId = $category->getId();
        }
    }

    protected function addCategoryForHadi($kategorie_id, &$errors) {
        $ids = $this->getCategoryIds($kategorie_id);
        foreach ($ids as $id) {
            $name = $this->getSurnameCode($id);
            if (!$name) {
                $errors .= " Not exist category name for Id = " . $id . " ";
                $log = $this->getLogger();
    ...
    public function importData(array $row) {
        ...
        if ($productExists == 0 && $this->addCategoryForHadi($kategorie_id, $errors)) { // product_numbers must be unique
            $kategorie_id = $this->parentId;
